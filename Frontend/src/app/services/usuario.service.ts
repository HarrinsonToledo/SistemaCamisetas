...
export class UserService {
  ...
  public async registrar(datosRegistro: any) {
    const envia = {
      method: 'POST',
    ...
    await fetch(`${this.myApiUrl}${this.propeties}${this.reg}`, envia)
      .then((response) => response.json())
      .then((data) => {
        if (data.statusCode == 404) {
          Notiflix.Loading.remove();
          Notiflix.Notify.warning("Error de registro");
        } else if (data.statusCode == 201) {
          Notiflix.Loading.remove();
          Notiflix.Notify.success("Registrado Logueado"); 
          this.loginState.createCookie(datosRegistro.nombre, data.correo)
          this.loginState.setLoginState()
          this.router.navigate(['/'])
        } else {
          console.log(data)
